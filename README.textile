...
* install imagemagick to handle the tiff to jp2 conversion <code>brew install imagemagick --with-ghostscript --with-openjpeg</code>
...
* In a separate tab, start Sidekiq <pre>bundle exec sidekiq</pre>
* In a separate tab, start Fedora and Solr <pre>rake jetty:start</pre> (if not already running)
...
* In a separate tab, start the Rails app, specifying port 3331 <pre>rails server -p 3331</pre> 
