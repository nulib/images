    ...
    describe "when logged in" do
      ...
      describe "on a group the user owns" do
        before do
          ...
          Hydra::LDAP.should_receive(:owner_for_group).with(@group.code).and_return(@user.uid)
          Hydra::LDAP.should_receive(:add_users_to_group).with(@group.code, ['ken']).and_return(true)
        end
      ...
      describe "on a group the user doesn't own" do
        before do
          ...
          Hydra::LDAP.should_receive(:owner_for_group).with(@group.code).and_return(@group.owner_uid)
        end
        it "should handle errors" do
          post :create, :id=>'ken', :group_id=>@group.id
    ...
    describe "when logged in" do
      ...
      describe "on a group the user owns" do
        before do
          @group= FactoryGirl.create(:user_group, :owner=>@user)
          Hydra::LDAP.should_receive(:owner_for_group).with(@group.code).and_return(@user.uid)
          Hydra::LDAP.should_receive(:remove_users_from_group).with(@group.code, ['ken']).and_return(true)
        end
        it "should be successful" do
      ...
      describe "on a group the user doesn't own" do
        before do
          @group= FactoryGirl.create(:user_group)
          Hydra::LDAP.should_receive(:owner_for_group).with(@group.code).and_return(@group.owner_uid)
          Hydra::LDAP.should_receive(:groups_for_user).with(@user.uid).and_return([@group])
        end
        it "should handle errors" do
          delete :destroy, :id=>'ken', :group_id=>@group.id
          response.should redirect_to(root_path)
