...
module PolicyAwareAbility
  ...
  def test_edit_from_policy(object_pid, user, session)    
    ...
    if policy_pid.nil?
      ...
      logger.debug("CANCAN Checking edit permissions for user: #{user} against policy #{policy_pid}")
  ...
  def test_read_from_policy(object_pid, user, session)
    ...
    if policy_pid.nil?
      ...
      logger.debug("CANCAN Checking read permissions for user: #{user} against policy #{policy_pid}")
